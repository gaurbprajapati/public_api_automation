...
public class GetPlacementsByIdAPI_Test extends TestBase {
	...
	public void getPlacementsByIdWithValidToken_PublicAPI(int placementId) {

		Map<String, String> pathParameters = new HashMap<>();
		pathParameters.put("id", String.valueOf(1));

		Response response = RestClient.doGet("JSON", baseURL, basePath, apiKeyA, null, pathParameters, true);

		response.then().statusCode(200);
		response.then().assertThat().body("id", Matchers.equalTo(1));
		response.then().assertThat().body("created_by", Matchers.equalTo(accountOwnerId));

		response.then().assertThat().body(matchesJsonSchemaInClasspath("publicApi//placement//getPlacementByIdAPI.json"));
	}

	@Owner("Sai Teja SG")
	@Test(groups = "nightly-build")
	public void getPlacementsByIdWithInvalidId_PublicAPI() {
		Map<String, String> pathParameters = new HashMap<>();
	...
	@Owner("Sai Teja SG")
	@Test(groups = "nightly-build")
	public void getPlacementsByIdWithInvalidToken_PublicAPI() {
		Map<String, String> pathParameters = new HashMap<>();
		pathParameters.put("id", String.valueOf(placementFaker.getRandomID()));

		Response response = RestClient.doGet("JSON", baseURL, basePath, apiKeyA + "123", null, pathParameters, true);
	...
	@Owner("Sai Teja SG")
	@Test(dataProvider = "getPlacementId", groups = "nightly-build")
	public void getPlacementsByIdWithCrossAccountToken_PublicAPI(int placementId) {
		Map<String, String> pathParameters = new HashMap<>();
		pathParameters.put("id", String.valueOf(placementId));

		Response response = RestClient.doGet("JSON", baseURL, basePath, apiKeyB, null, pathParameters, true);
