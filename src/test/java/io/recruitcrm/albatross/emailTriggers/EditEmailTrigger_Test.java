...
public class EditEmailTrigger_Test extends TestBase {
	...
	public void postEditEmailTriggerTest_200(int trigger, int stageId, int id, String name, String assertionMsg) {

		Emailtriggersetting emailtriggersetting = new Emailtriggersetting(id, fakerTrigger.getTriggerName(), trigger,
				stageId, 0);
		NewEmailTrigger newEmailTrigger = new NewEmailTrigger(emailtriggersetting);
		Map<String, String> pathParamters = new HashMap<String, String>();
		pathParamters.put("id", String.valueOf(id));
		Response response = RestClient.doPost1("JSON", albatrossURL, "email-triggers/{id}", albatrossToken, null,
				pathParamters, true, newEmailTrigger);
		Assert.assertEquals("Request Failure", response.statusCode(), 200);
		JsonPath jsonPath = response.jsonPath();
		Assert.assertEquals("Response body differs", jsonPath.get("message"), "Update Email Trigger Successful ");
	...
	@Owner("Divya")
	@Test(dataProvider = "getTriggerId", groups = "nightly-build")
	public void postEditEmailTriggerWithInvalidValuesTest_422(int trigger, int stageId, int id, String name,
			String assertionMsg) {

		Emailtriggersetting emailtriggersetting = new Emailtriggersetting(id, name, fakerTrigger.getTriggerId(),
				stageId, 0);
		NewEmailTrigger newEmailTrigger = new NewEmailTrigger(emailtriggersetting);
		Map<String, String> pathParamters = new HashMap<String, String>();
		pathParamters.put("id", String.valueOf(id));
	...
	public void postUnauthorizedAccessEditEmailTriggerTest_401() {

		Emailtriggersetting emailtriggersetting = new Emailtriggersetting(null, 0, 0, 0);
		NewEmailTrigger newEmailTrigger = new NewEmailTrigger(emailtriggersetting);
		Map<String, String> pathParamters = new HashMap<String, String>();
		pathParamters.put("id", String.valueOf(fakerTrigger.getTriggerId()));
		Response response = RestClient.doPost1("JSON", albatrossURL, "email-triggers/{id}", albatrossToken + "123",
				null, pathParamters, true, newEmailTrigger);
	...
	public Object[][] getTriggerId() {
		...
		JsonPath jsonPath = RestClient
				...
				.jsonPath();
		int triggerId = jsonPath.get("data.records[0].id");
		Object[][] data = {
				{ 4, hiringStageId, triggerId, fakerTrigger.getTriggerName(), "Failed To Update Email Trigger" },
				{ 4, hiringStageId, triggerId, null, "The emailtriggersetting.name field is required." } };
		return data;
	}
}
