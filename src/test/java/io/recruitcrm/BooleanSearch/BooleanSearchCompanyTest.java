...
public class BooleanSearchCompanyTest extends FilterSearchBaseTest {
    ...
    @Owner("Raj Pandey")
    @Test(groups = {"aries_service"}, dataProvider = "booleanSearchCompanyTestData", description = "Boolean Search Test for Company")
    public void booleanSearchCompanyTest(String testCaseId, String description, String filterValue, String entities, String expectedResult, String expectedResultName, String expectedResultReason) {
        ...
        FilterSearchReporter.logFieldValues(response, data, "Name", "companyname");
    ...
    public JSONObject createBooleanSearchPayload(String filterValue, String entities) {
        Set<String> validEntities = new HashSet<>(Arrays.asList(
            "notes", "call_logs", "tasks", "meetings", "companies"
        ));
    ...
    @DataProvider(name = "booleanSearchCompanyTestData")
    public Object[][] booleanSearchCompanyTestData() {
        String filePath = "src/test/resources/filtersDataProvider/companyBooleanSearchDataProvider.json";
    ...
    public void validateBooleanSearchResults(JSONArray data, String testCaseId, String description, 
        ...
        if (expectedResult.equals("Empty")) {
            Assert.assertEquals(data.length(), 0, "Test Case: " + testCaseId + " - Expected empty result but got " + data.length() + " companies for query: " + filterValue);
            return;
        }
        
        if (data.length() == 0) {
            Assert.fail("Test Case: " + testCaseId + " - Expected results but got 0 companies for query: " + filterValue);
        }
        
        // Parse expected result (comma-separated company keys)
        String[] expectedCompanyKeys = expectedResult.split(",");
        List<Integer> expectedCompanyIds = new ArrayList<>();
        
        for (String companyKey : expectedCompanyKeys) {
            String normalizedKey = companyKey.trim().toLowerCase().replace(" ", "");
            String companyIdStr = companyKeyToIdMap.get(normalizedKey);
            if (companyIdStr == null) {
                Assert.fail("Test Case: " + testCaseId + " - Expected company key '" + companyKey + "' (normalized: '" + normalizedKey + "') not found in companyKeyToIdMap. Available keys: " + companyKeyToIdMap.keySet());
            }
            expectedCompanyIds.add(Integer.parseInt(companyIdStr));
        }
        
        // Extract actual company IDs from response
        ...
        Assert.assertEquals(data.length(), expectedCompanyIds.size(), "Test Case: " + testCaseId + " - Expected " + expectedCompanyIds.size() + " companies but got " + data.length() + " for query: " + filterValue);
        
        // Validate each expected company is present
        for (int companyId : expectedCompanyIds) {
            if (!actualCompanyIds.contains(companyId)) {
                String companyKey = companyIdToKeyMap.get(String.valueOf(companyId));
                Assert.fail("Test Case: " + testCaseId + " - Company: " + companyKey + " (ID: " + companyId + ") is not present in the actual response but was expected to be present. Query: " + filterValue);
            }
        }
        
        // Validate no extra companies
        for (int companyId : actualCompanyIds) {
            if (!expectedCompanyIds.contains(companyId)) {
                String companyKey = companyIdToKeyMap.get(String.valueOf(companyId));
                Assert.fail("Test Case: " + testCaseId + " - Company: " + companyKey + " (ID: " + companyId + ") is present in the actual response but was not expected. Query: " + filterValue);
            }
        }
    }

    public void createTestData() {
        JSONObject companyJson = readJsonFileFromPath("src/test/resources/testData/booleanSearchCompany_data.json");
        ...
        try {
            ...
            createNotesForCompany(companySlugMap);
            createTasksForCompany(companySlugMap);
            createCallLogForCompany(companySlugMap);
            createMeetingsForCompany(companySlugMap);
        } finally {
            executor.shutdown();
        }
    }
            ...
            List<CompletableFuture<AbstractMap.SimpleEntry<String, Response>>> futures = entityTypes.stream()
                .map(entity -> CompletableFuture.supplyAsync(() -> {
                    ...
                    if (entity.equals("dropdown") || entity.equals("multiselect")) {
                        ...
                        response = function.createCustomFieldsResponse(albatrossURL, albatrossAuthToken, "company", fieldName, entity, "");
                    }
    ...
    private void createAssociatedEntities() {
        ...
        try {
            ...
            // Allow expectedResult in JSON to reference the same IDs as note/task/call/meeting associated_companies placeholders
            String assocCo1Id = String.valueOf(associatedEntitiesIdMap.get("associated_companies_company1"));
            String assocCo2Id = String.valueOf(associatedEntitiesIdMap.get("associated_companies_company2"));
            companyKeyToIdMap.put("associated_companies_company1", assocCo1Id);
            companyKeyToIdMap.put("associated_companies_company2", assocCo2Id);
            companyIdToKeyMap.put(assocCo1Id, "associated_companies_company1");
            companyIdToKeyMap.put(assocCo2Id, "associated_companies_company2");
        } finally {
            executor.shutdown();
        }
    }
        ...
        try {
            ...
            for (String companyKey : companySlugMap.keySet()) {
                ...
                futures.add(CompletableFuture.runAsync(() -> {
                    ...
                    JSONObject processed = processNotePayload(notesData, companySlug);
    ...
    private JSONObject processNotePayload(JSONObject notesData, String companySlug) {
        ...
        note.put("related_to", companySlug);
        note.put("related_to_type", "company");
    ...
    private List<Integer> createCallLogForCompany(Map<String, String> companySlugMap) {
        JSONObject companyJson = readJsonFileFromPath("src/test/resources/testData/booleanSearchCompany_data.json");
        ...
        try {
            ...
            for (String companyKey : companySlugMap.keySet()) {
                if (!companyJson.has(companyKey) || !companyJson.getJSONObject(companyKey).has("callLogs")) {
                    continue;
                }
                ...
                futures.add(CompletableFuture.runAsync(() -> {
                    String companySlug = companySlugMap.get(companyKey);
                    JSONObject companyEntry = companyJson.getJSONObject(companyKey);
                    JSONObject callLogsData = companyEntry.getJSONObject("callLogs");
                    JSONObject payload = processCallLogPayload(callLogsData, companySlug);
                    Response response = function.createCallLogByPayload(baseURL, accountOwnerAPIKey, payload.getJSONArray("calls").getJSONObject(0));
                    callLogIdMap.put(companyKey, response.jsonPath().getInt("id"));
                }, executor));
            }
    ...
    private JSONObject processCallLogPayload(JSONObject payload, String companySlug) {
        if (payload.has("calls") && payload.get("calls") instanceof org.json.JSONArray) {
            ...
            for (int i = 0; i < callsArray.length(); i++) {
                ...
                if (call.has("related_to") && call.getString("related_to").startsWith("{company")) {
                    call.put("related_to", companySlug);
                }
    ...
    @Owner("Raj Pandey")
    @Test(groups = {FilterSearchBaseTest.ARIES_SERVICE_SMOKE_GROUP, "nightly-build"}, dataProvider = "booleanSearchCompanySmokeTestData", description = "[Smoke] Boolean Search Test for Company")
    public void booleanSearchCompanySmokeTest(String testCaseId, String description, String filterValue, String entities, String expectedResult, String expectedResultName, String expectedResultReason) {
        booleanSearchCompanyTest(testCaseId, description, filterValue, entities, expectedResult, expectedResultName, expectedResultReason);
    }

    @DataProvider(name = "booleanSearchCompanySmokeTestData", parallel = true)
    public Object[][] booleanSearchCompanySmokeTestData() {
        return limitSmokeRows(booleanSearchCompanyTestData());
    }
}
