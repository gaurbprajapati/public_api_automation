package io.recruitcrm.ostrich;

import java.util.ArrayList;

import io.rcrm.api.pojo.ostrich.BulkDelete;
import org.hamcrest.Matchers;
import org.testng.Assert;
import org.testng.annotations.Test;

import com.qa.api.util.reaper.ThreadManager;

import io.rcrm.api.commanfunctions.commanFunction;
import io.rcrm.api.pojo.ostrich.Tasks;
import io.rcrm.api.restclient.RestClient;
import io.rcrm.api.testbase.TestBase;
import io.rcrm.api.testbase.TestBase.AccountType;
import io.restassured.response.Response;

import static io.restassured.module.jsv.JsonSchemaValidator.matchesJsonSchemaInClasspath;
import com.qa.api.util.Owner;

@AccountType("Business|AlbatrossTkn")
public class BulkDeleteTaskTest extends TestBase {
    commanFunction function = new commanFunction();

    @Owner("Harika")
    @Test
    public void bulkDeleteTasks() {
        ArrayList<Integer> taskIDs = new ArrayList<>();
        for (int i = 0; i < 2; i++) {
            int taskId = function.createNewTask(baseURL, ThreadManager.getAccountApiKey(), "candidate").jsonPath()
                    .get("id");
            taskIDs.add(taskId);
        }
        BulkDelete tasks = new BulkDelete();
        tasks.setIdsToDelete(taskIDs);
        tasks.setType(1);

        String basePath = "/tasks-and-meetings/bulk-delete";
        Response response = RestClient.doPost("JSON", ostrichURL, basePath, ThreadManager.getOwnerAlbatrossToken(),
                null, true, tasks);

        Assert.assertEquals(response.getStatusCode(), 200);
        response.then().assertThat()
                .body(matchesJsonSchemaInClasspath("privateApi/activities/bulkDeleteActivity.json"));
        response.then().body("message_type", Matchers.containsString("is-success"));
        response.then().body("message", Matchers.containsString("Records deleted successfully"));
        response.then().body("data", Matchers.is(2));
    }

    @Owner("Harika")
    @Test
    public void bulkDeleteTasks_401() {
        ArrayList<Integer> taskIDs = new ArrayList<>();
        for (int i = 0; i < 2; i++) {
            int taskId = function.createNewTask(baseURL, ThreadManager.getAccountApiKey(), "candidate").jsonPath()
                    .get("id");
            taskIDs.add(taskId);
        }
        BulkDelete tasks = new BulkDelete();
        tasks.setIdsToDelete(taskIDs);
        tasks.setType(1);

        String basePath = "/tasks-and-meetings/bulk-delete";
        Response response = RestClient.doPost("JSON", ostrichURL, basePath,
                ThreadManager.getOwnerAlbatrossToken() + "12345", null, true, tasks);

        response.then().statusCode(401);
        response.then().body("error", Matchers.containsString("Unauthorized"));
    }

    @Owner("Harika")
    @Test
    public void bulkDeleteTasks_422() {
        BulkDelete tasks = new BulkDelete();
        tasks.setType(1);

        String basePath = "/tasks-and-meetings/bulk-delete";
        Response response = RestClient.doPost("JSON", ostrichURL, basePath, ThreadManager.getOwnerAlbatrossToken(),
                null, true, tasks);

        Assert.assertEquals(response.getStatusCode(), 422);
    }

}
