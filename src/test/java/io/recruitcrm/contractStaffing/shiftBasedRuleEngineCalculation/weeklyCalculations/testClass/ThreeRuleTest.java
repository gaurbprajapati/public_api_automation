...
import static io.recruitcrm.contractStaffing.common.TimesheetTestDataLoader.TestSuite.SHIFT_THREE_RULE;
import com.qa.api.util.Owner;

/**
 * Three-rule test: runs full flow (template → entities → timesheet → multi-entry update → evaluate)
 * driven by ThreeRuleTest.json test data.
 */
@TestBase.AccountType("Business|AlbatrossTkn|contractStaffing")
public final class ThreeRuleTest extends MultipleTimeEntryBaseTest {
    ...
    @Owner("Yash Rampal")
    @Test(dataProvider = "threeRuleDataProvider")
    public void verifyThreeRuleMultipleTimeEntry(Map<String, Object> scenario) {
        executeMultipleTimeEntryTest(scenario, albatrossAuthToken, apiAuthToken, function, createdTemplateIds);
    }

    @DataProvider(name = "threeRuleDataProvider", parallel = true)
    public Object[][] provideThreeRuleTestData() {
