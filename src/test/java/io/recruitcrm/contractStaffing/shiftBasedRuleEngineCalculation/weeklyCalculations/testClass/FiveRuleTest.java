...
import static io.recruitcrm.contractStaffing.common.TimesheetTestDataLoader.TestSuite.SHIFT_FIVE_RULE;
import com.qa.api.util.Owner;

/**
 * Five-rule test: runs full flow (template → entities → timesheet → multi-entry update → evaluate)
 * driven by FiveRuleTest.json test data.
 */
@TestBase.AccountType("Business|AlbatrossTkn|contractStaffing")
public final class FiveRuleTest extends MultipleTimeEntryBaseTest {
    ...
    @Owner("Yash Rampal")
    @Test(dataProvider = "fiveRuleDataProvider")
    public void verifyFiveRuleMultipleTimeEntry(Map<String, Object> scenario) {
        executeMultipleTimeEntryTest(scenario, albatrossAuthToken, apiAuthToken, function, createdTemplateIds);
    }

    @DataProvider(name = "fiveRuleDataProvider",parallel = true)
    public Object[][] provideFiveRuleTestData() {
