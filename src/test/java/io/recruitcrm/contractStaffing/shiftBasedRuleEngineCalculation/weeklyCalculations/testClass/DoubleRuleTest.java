...
import static io.recruitcrm.contractStaffing.common.TimesheetTestDataLoader.TestSuite.SHIFT_DOUBLE_RULE;
import com.qa.api.util.Owner;

/**
 * Double-rule test: runs full flow (template → entities → timesheet → multi-entry update → evaluate)
 * driven by DoubleRuleTest.json test data.
 */
@TestBase.AccountType("Business|AlbatrossTkn|contractStaffing")
public final class DoubleRuleTest extends MultipleTimeEntryBaseTest {
    ...
    @Owner("Yash Rampal")
    @Test(dataProvider = "doubleRuleDataProvider")
    public void verifyDoubleRuleMultipleTimeEntry(Map<String, Object> scenario) {
        executeMultipleTimeEntryTest(scenario, albatrossAuthToken, apiAuthToken, function, createdTemplateIds);
    }

    @DataProvider(name = "doubleRuleDataProvider",parallel = true)
    public Object[][] provideDoubleRuleTestData() {
