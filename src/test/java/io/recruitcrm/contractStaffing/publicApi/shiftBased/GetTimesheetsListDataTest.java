...
public class GetTimesheetsListDataTest extends ContractStaffingBaseTest {
    ...
    private double expectedPayAmount;
    private double expectedBillAmount;

    private static final int EXPECTED_TOTAL_TIMESHEETS = 13;
    ...
    private void generateTimesheetData() {
        ...
        JSONObject evalPayload = new JSONObject();
        evalPayload.put("timesheetId", timesheetId);
        Response evalResponse = RestClient.doPost("JSON", timesheetBaseURL, "rule-engine/evaluate",
                albatrossAuthToken, null, true, evalPayload);
        if (evalResponse.statusCode() == 200 || evalResponse.statusCode() == 201) {
            JsonPath evalJson = evalResponse.jsonPath();
            expectedPayAmount = evalJson.getDouble("data.evaluationSummary.totalPayAmount");
            expectedBillAmount = evalJson.getDouble("data.evaluationSummary.totalBillAmount");
        }
        ...
        if (slugResponse.getStatusCode() == 200) {
    ...
    /**
     * Counts list items whose {@code timesheet_status.id} equals {@code statusId}.
     */
    ...
    /**
     * Asserts each row: structure, then submitted-specific values (id=2) or open-specific values (id=1).
     * Does not assume {@code data[0]} is submitted; sort order can place any timesheet first.
     */
    private void assertFullListSubmittedOpenPattern(Response response, List<Map<String, Object>> data,
        ...
        assertThat(tidPrefix + " - open count (id=1)", countTimesheetsByStatusId(response, data, 1),
                is(EXPECTED_TOTAL_TIMESHEETS - 1));
    ...
    private void assertTimesheetItemStructure(Response response, String prefix, String tid) {
        ...
        assertThat(prefix + ".pay.rate (" + tid + ")", response.jsonPath().getInt(prefix + ".pay.rate"), is(5000));
        ...
        assertThat(prefix + ".bill.rate (" + tid + ")", response.jsonPath().getInt(prefix + ".bill.rate"), is(6000));
    ...
    private void assertSubmittedTimesheetValues(Response response, String prefix, String tid) {
        ...
        assertThat(prefix + ".hours.total_regular (" + tid + ")", response.jsonPath().getString(prefix + ".hours.total_regular"), notNullValue());
        double actualPay = response.jsonPath().getDouble(prefix + ".pay.amount");
        double actualBill = response.jsonPath().getDouble(prefix + ".bill.amount");
        assertThat(prefix + ".pay.amount (" + tid + ") expected=" + expectedPayAmount + " actual=" + actualPay,
                Math.abs(actualPay - expectedPayAmount) <= 1.0, is(true));
        assertThat(prefix + ".bill.amount (" + tid + ") expected=" + expectedBillAmount + " actual=" + actualBill,
                Math.abs(actualBill - expectedBillAmount) <= 1.0, is(true));
    }
    ...
    public void getTimesheetsListWithAllCorrectParamsTest() {
        ...
        assertThat("data length", data.size(), is(EXPECTED_TOTAL_TIMESHEETS));
    ...
    public void getTimesheetsListByCandidateSlugTest(String testCase, String candidateSlugParam,
        ...
        if (expectData) {
            List<Map<String, Object>> data = response.jsonPath().getList("data");
            assertThat(testCase + " - data length", data.size(), is(EXPECTED_TOTAL_TIMESHEETS));
    ...
    public void getTimesheetsListByTimesheetStatusTest(String testCase, String statusParam,
        ...
        if (expectedStatus == 200) {
            ...
            if ("submitted".equals(statusParam)) {
                ...
                assertSubmittedTimesheetValues(response, "data[0]", testCase);
            }
            if ("open".equals(statusParam)) {
                assertThat(testCase + " - open count", data.size(), is(EXPECTED_TOTAL_TIMESHEETS - 1));
                for (int i = 0; i < data.size(); i++) {
    ...
    public Object[][] timesheetStatusDataProvider() {
        return new Object[][] {
                { "Status open", "open", 200, EXPECTED_TOTAL_TIMESHEETS - 1 },
                ...
                { "Empty status", "", 200, EXPECTED_TOTAL_TIMESHEETS },
        };
    }
