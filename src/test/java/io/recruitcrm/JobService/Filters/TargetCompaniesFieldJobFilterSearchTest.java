...
public class TargetCompaniesFieldJobFilterSearchTest extends FilterSearchBaseTest {
    ...
    @Owner("Raj Pandey")
    @Test(groups = {"aries_service"}, dataProvider = "targetCompaniesFieldFilterJobSearchTestData", description = "Filter Search Test for Target Companies Field")
    public void targetCompaniesFieldFilterSearchTest(String fieldName, String filterType, String filterValue, String dbField, String expectedResult, String fieldType, String filterValue_TYPE, String testCaseId) {
    ...
    @DataProvider(name = "targetCompaniesFieldFilterJobSearchTestData", parallel = true)
    public Object[][] dataProvider() {
        JSONObject filterData = readJsonFileFromPath("src/test/resources/filtersDataProvider/jobTargetCompaniesFilterDataProvider.json");
    ...
    public JSONObject createFilterSearchPayload(String fieldName, String filterType, String filterValue, String dbField, String fieldType, String filterValue_TYPE) {
        ...
        List<Integer> companyIds = parseCompanyIds(filterValue);
    ...
    private List<Integer> parseCompanyIds(String filterValue) {
        ...
        for (String raw : filterValue.split(",")) {
            ...
            // Handle company placeholders like {company1_id} or {company1}
            String companyKey = key;
            if (key.endsWith("_id")) {
                companyKey = key.substring(0, key.length() - 3);
            }
    
            String companyIdStr = companyKeyToIdMap.get(companyKey);
            //If company ID is not found, fail the test
            if (companyIdStr == null) {
                Assert.fail("Wrong company name provided in the filter value. Company ID not found for: " + placeholder + ". Available companies: " + companyKeyToIdMap.keySet());
            }
            companyIds.add(Integer.parseInt(companyIdStr));
        }
