...
public class GetContactHotlistCountTest extends TestBase {
    String apiAuthToken;
    ...
    public void getContactHotlistCountTest_Success() {
        String basePath = "advanced-search/contact-hotlists/search/count/get";
    ...
    @Owner("Raj Pandey")
    @Test(groups = {"contact_service", "nightly-build"})
    public void getContactHotlistCountTest_WithoutAuth() {
        String basePath = "advanced-search/contact-hotlists/search/count/get";
        Map<String, String> queryParameters = new HashMap<>();
    ...
    @Owner("Raj Pandey")
    @Test(groups = {"contact_service", "nightly-build"})
    public void getContactHotlistCountTest_InvalidAuth() {
        String basePath = "advanced-search/contact-hotlists/search/count/get";
        Map<String, String> queryParameters = new HashMap<>();
    ...
    @Owner("Raj Pandey")
    @Test(dataProvider = "getContactHotlistCountData", groups = {"contact_service", "nightly-build"})
    public void getContactHotlistCountTest_DataProvider(String dbField, String filterType, String filterValue, String fieldType, int count, String token) {
        String basePath = "advanced-search/contact-hotlists/search/count/get";
        Map<String, String> queryParameters = new HashMap<>();
    ...
    @DataProvider(name = "getContactHotlistCountData", parallel = true)
    public Object[][] getContactHotlistCountData() {
        JSONObject filterData = readJsonFileFromPath("src/test/resources/filtersTest/contactHotlistCountDataProvider.json");
    ...
    private JSONObject createHotlistCountPayload(String sortField, String sortOrder) {
        ...
        payload.put("advancedSearchContext", "CONTACT_HOTLIST");
    ...
    public void createData() {
        JSONObject hotlistData = readJsonFileFromPath("src/test/resources/contact_hotlistData.json");
        Map<String, Map<String, String>> hotlistTimestampScenarios = commanFunction.createTimestampScenarios();
        ...
        while (keys.hasNext()) {
            ...
            if (!createdOn.isEmpty() && !updatedOn.isEmpty()) {
                updateHotlistTimestamps(hotlistId, createdOn, updatedOn, hotlistKey, hotlistTimestampScenarios);
            }
        }
    }

    private void updateHotlistTimestamps(int hotlistId, String createdOnScenario, String updatedOnScenario, String hotlistKey, Map<String, Map<String, String>> hotlistTimestampScenarios) {
        Map<String, String> createdOnTimestamps = hotlistTimestampScenarios.get(createdOnScenario);
        if (createdOnTimestamps == null) {
            Assert.fail("Timestamp scenario not found for createdOn key: " + createdOnScenario + ". Available scenarios: " + hotlistTimestampScenarios.keySet());
        }
        
        Map<String, String> updatedOnTimestamps = hotlistTimestampScenarios.get(updatedOnScenario);
        if (updatedOnTimestamps == null) {
            Assert.fail("Timestamp scenario not found for updatedOn key: " + updatedOnScenario + ". Available scenarios: " + hotlistTimestampScenarios.keySet());
        }
        
        String createdOnTimestamp = createdOnTimestamps.get("createdOn");
        String updatedOnTimestamp = updatedOnTimestamps.get("updatedOn");
