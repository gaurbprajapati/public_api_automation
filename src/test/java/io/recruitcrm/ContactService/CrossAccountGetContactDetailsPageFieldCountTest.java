...
public class CrossAccountGetContactDetailsPageFieldCountTest extends TestBase {
    ...
    private void setupUserIds() {
        ...
        Response getUsersA = commanFunction.getUsers(baseURL, accountA_apiKey);
        assertThat("Failed to get users", getUsersA.getStatusCode(), equalTo(200));
        JsonPath jpA = getUsersA.jsonPath();
        accountA_UserId = jpA.get("[0].id");
        assertThat("Account A User ID should not be null", accountA_UserId, notNullValue());

        Response getUsersB = commanFunction.getUsers(baseURL, accountA_apiKey);
        assertThat("Failed to get users", getUsersB.getStatusCode(), equalTo(200));
        JsonPath jpB = getUsersA.jsonPath();
        accountB_UserId = jpB.get("[0].id");
        assertThat("Account A User ID should not be null", accountB_UserId, notNullValue());
    }

    @Owner("Harika")
    @Test(dataProvider = "crossAccountContactDetailsPageFieldsCountTestData", groups = {"contact_service", "nightly-build"})
    public void crossAccountContactDetailsPageFieldsCountOperations_Test(String testScenario, String accountType, String tokenType,
        ...
        try {
            switch (operation.toUpperCase()) {
                case "GET_CONTACT_DETAILS_PAGE_FIELDS_COUNT_A":
                    response = getContactDetailsPageFieldsCount(accountA_AccountId, accountA_UserId, token);
                    break;
                    
                case "GET_CONTACT_DETAILS_PAGE_FIELDS_COUNT_CROSS_ACCOUNTID":
                    response = getContactDetailsPageFieldsCount(accountA_AccountId, accountB_UserId, token);
                    break;
                
                case "GET_CONTACT_DETAILS_PAGE_FIELDS_COUNT_CROSS_USERID":
                    response = getContactDetailsPageFieldsCount(accountB_AccountId, accountA_UserId, token);
                    break;
    ...
    private Response getContactDetailsPageFieldsCount(int accountId, int userId, String token) {
    ...
    @DataProvider(name = "crossAccountContactDetailsPageFieldsCountTestData", parallel = true)
    public static Object[][] crossAccountContactDetailsPageFieldsCountTestData() {
        return new Object[][] {

            {"SCENARIO_2_CROSS_ACCOUNT_ACCESS_B_TO_A", "AccountB", "valid",
                "GET_CONTACT_DETAILS_PAGE_FIELDS_COUNT_CROSS_ACCOUNTID", "401", "Unauthorized",
                "Account B should not access Account A's contact details page field count"},

            {"SCENARIO_3_CROSS_ACCOUNT_ACCESS_B_TO_A", "AccountB", "valid",
                "GET_CONTACT_DETAILS_PAGE_FIELDS_COUNT_CROSS_USERID", "401", "Unauthorized",
                "Account B should not access Account A's contact details page field count"},

            {"SCENARIO_5_NONEXISTENT_ACCOUNT", "AccountC", "valid", "GET_CONTACT_DETAILS_PAGE_FIELDS_COUNT_A",
                "401", "Unauthorised access", "Non-existent account should return 401"},

            {"SCENARIO_6_EXPIRED_TOKEN", "AccountA", "expired", "GET_CONTACT_DETAILS_PAGE_FIELDS_COUNT_A",
                "401", "Unauthorised access", "Expired token should return 401"},

            {"SCENARIO_7_MALFORMED_TOKEN", "AccountA", "malformed", "GET_CONTACT_DETAILS_PAGE_FIELDS_COUNT_A",
                "401", "Unauthorised access", "Malformed token should return 401"},

            {"SCENARIO_9_NULL_TOKEN", "AccountA", "null", "GET_CONTACT_DETAILS_PAGE_FIELDS_COUNT_A",
