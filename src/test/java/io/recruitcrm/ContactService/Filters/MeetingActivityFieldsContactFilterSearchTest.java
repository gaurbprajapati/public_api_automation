...
public class MeetingActivityFieldsContactFilterSearchTest extends FilterSearchBaseTest {
    ...
    ConcurrentHashMap<Integer, String> meetingIdToContactKeyMap = new ConcurrentHashMap<>();
    ...
    public Object[][] meetingsDateFieldDataProvider() {
        JSONObject filterData = readJsonFileFromPath("src/test/resources/filtersDataProvider/contactMeetingFilterDataProvider.json");
    ...
    @Owner("Raj Pandey")
    @Test(groups = {"aries_service"}, dataProvider = "meetingsDateFieldFilterSearchTestData", description = "Meetings Activity Fields Contact Filter Search Test")
    ...
    @Owner("Sai Teja SG")
    @Test(groups = {"aries_service"}, dataProvider = "meetingsMeetingExistsFilterSearchTestData", description = "Meetings Meeting Exists (checkbox) Contact Filter Search Test")
    ...
    public void validateDateFilteredData(JSONArray data, String filterType, String filterValue, String fieldName, String dbField, String expectedResult) {
        if(expectedResult.equals("Empty")) {
            Assert.assertEquals(data.length(), 0, "Expected empty result but got " + data.length() + " contacts for field: " + fieldName + " and filterType: " + filterType + " and filterValue: " + filterValue);
            return;
        ...
        if(data.length() == 0) {
            Assert.fail("Expected results but got 0 contacts for field: " + fieldName + " and filterType: " + filterType + " and filterValue: " + filterValue);
        }
        ...
        for (int i = 0; i < data.length(); i++) {
            ...
            Response meetingResponse = getContactMeetingsBySlug(albatrossURL, albatrossAuthToken, contactSlug);
            ...
            Assert.assertTrue(foundMatchingRecord, 
                "No matching meeting record found for contact ID: " + contactSlug + 
                " with field: " + fieldName + ", filterType: " + filterType + ", filterValue: " + filterValue);
        }
    }

    public Response getContactMeetingsBySlug(String albatrossURL, String authToken, String contactSlug) {
        ...
        Assert.assertEquals(response.getStatusCode(), 200, "Failed to fetch contact meetings data");
        return response;
    }
    ...
    @Owner("Raj Pandey")
    @Test(groups = {"aries_service"}, dataProvider = "meetingsMultiselectAndDropdownFilterSearchTest", description = "Meetings Multiselect Fields Contact Filter Search Test")
    ...
    @Owner("Raj Pandey")
    @Test(groups = {"aries_service"}, dataProvider = "meetingsTextFieldFilterSearchTest", description = "Meetings Text Fields Contact Filter Search Test")
    ...
    public void validateTextFieldFilteredData(JSONArray data, String filterType, String filterValue, String fieldName, String dbField, String expectedResult) {
        // First validate the count of contacts returned
        if(expectedResult.equals("Empty")) {
            Assert.assertEquals(data.length(), 0, "Expected empty result but got " + data.length() + " contacts for field: " + fieldName + " and filterType: " + filterType + " and filterValue: " + filterValue);
            return;
        } else if (expectedResult.isEmpty() || expectedResult.trim().isEmpty()){
        ...
        // Validate each contact's meetings
        for (int i = 0; i < data.length(); i++) {
            JSONObject contact = data.getJSONObject(i);
            int contactId = contact.getInt("id");
            String contactSlug = contact.getString("slug");
            ...
            try {
                ...
                Assert.assertEquals(meetingResponse.getStatusCode(), 200, 
                    "Failed to fetch meeting for contact ID: " + contactId);
                ...
                Assert.assertTrue(foundMatchingRecord, 
                    "No matching meeting record found for contact ID: " + contactId +
                    " with field: " + fieldName + ", filterType: " + filterType + ", filterValue: " + filterValue);
                    
            } catch (Exception e) {
                Assert.fail("Error validating meeting for contact ID: " + contactId +
                    " - " + e.getMessage());
            }
        }
    }
    ...
    public void createTestData() {
        ...
        try {
            ...
            List<Integer> meetingIds = createMeetingsForContact(contactSlugMap);
                ...
                futures.add(CompletableFuture.runAsync(() -> {
                    try {
                        ...
                        if (response.getStatusCode() == 200) {
                            ...
                            meetingIdToContactKeyMap.put(meetingId, contactKey);
                        } else {
                            System.err.println("Meeting creation failed for " + contactKey + ": " + response.getBody().asString());
                        }
                    } catch (Exception e) {
    ...
    private Meeting processMeetingPayload(JSONObject payload, String contactSlug) {
        ...
        String startDate = getVariedStartDateForContact(contactSlug);
        ...
        try {
            ...
            for (int i = 0; i < limit; i++) {
                ...
                futures.add(CompletableFuture.runAsync(() -> {
                    ...
                    String contactKey = meetingIdToContactKeyMap.get(meetingId);
                    ...
                    if (updateResponse.getStatusCode() != 200) {
                        System.err.println("Failed to update timestamps for " + contactKey + " (Meeting ID: " + meetingId + ")");
    ...
    private String getVariedStartDateForContact(String contactSlug) {
        String contactKey = contactSlugMap.entrySet().stream()
            .filter(entry -> entry.getValue().equals(contactSlug))
