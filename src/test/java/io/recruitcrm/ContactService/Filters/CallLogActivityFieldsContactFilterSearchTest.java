...
public class CallLogActivityFieldsContactFilterSearchTest extends FilterSearchBaseTest {
    ...
    public Object[][] callLogDateFieldDataProvider() {
        JSONObject filterData = readJsonFileFromPath("src/test/resources/filtersDataProvider/contactCallLogFilterDataProvider.json");
    ...
    @Owner("Raj Pandey")
    @Test(groups = {"aries_service"}, dataProvider = "callLogDateFieldFilterSearchTestData", description = "Call Log Activity Fields Contact Filter Search Test")
    public void callLogDateFieldFilterSearchTest(String fieldName, String filterType, String filterValue, String dbField, String expectedResult, String fieldType, String filterValue_TYPE, String testCaseId) {
        ...
        for (String key : filterData.keySet()) {
            ...
            for (int i = 0; i < tests.length(); i++) {
                ...
                if ("checkbox".equals(fieldType)) {
                    ...
                    String testCaseId = test.optString("testCaseId", "");
                    testData.add(new Object[]{
                            key, filterType, filterValue, test.getString("dbField"), test.getString("expectedResult"), fieldType, testCaseId
                    });
                }
            }
        }
    ...
    @Owner("Sai Teja SG")
    @Test(groups = {"aries_service"}, dataProvider = "callLogCallExistsFilterSearchTestData", description = "Call Log Exists (checkbox) Contact Filter Search Test")
    public void callLogCallExistsFilterSearchTest(String fieldName, String filterType, String filterValue, String dbField, String expectedResult, String fieldType, String testCaseId) {
        FilterSearchReporter.logInfo("Test Case ID: ", testCaseId);
        FilterSearchReporter.logInfo("Account: ", email);
        String basePath = "/contacts/search/get?page=1&size=100";
        JSONObject payload = createMultiselectAndDropdownFilterSearchPayload(fieldName, filterType, filterValue, dbField, fieldType);
        FilterSearchReporter.logPayload(payload);
    ...
    public void validateDateFilteredData(JSONArray data, String filterType, String filterValue, String fieldName, String dbField, String expectedResult) {
        for (int i = 0; i < data.length(); i++) {
            ...
            Response callLogResponse = getContactCallLogsBySlug(albatrossURL, albatrossAuthToken, contactSlug);
            String responseBody = callLogResponse.getBody().asString();
            ...
            if (filterType.equals("is_empty") && notes.length() == 0) {
                foundMatchingRecord = true;
            }
            for (int j = 0; j < notes.length(); j++) {
                JSONObject note = notes.getJSONObject(j);
                // Skip notes (type == 0), only process call logs (type == 1)
                if (note.getInt("type") == 0) {
                    continue;
                }
                String callLogDate = note.optString(dbField, "0");

                if (validateDateAgainstFilter(callLogDate, filterType, filterValue, fieldName)) {
                    foundMatchingRecord = true;
                    break; 
                }
            ...
            Assert.assertTrue(foundMatchingRecord, 
                "No matching call log record found for contact ID: " + contactSlug + 
                " with field: " + fieldName + ", filterType: " + filterType + ", filterValue: " + filterValue);
        }
    }

    public Response getContactCallLogsBySlug(String albatrossURL, String authToken, String contactSlug) {
        ...
        Assert.assertEquals(response.getStatusCode(), 200, "Failed to fetch contact call logs data");
        return response;
    }
    ...
    @Owner("Raj Pandey")
    @Test(groups = {"aries_service"}, dataProvider = "callLogMultiselectAndDropdownFilterSearchTest", description = "Call Log Activity Fields Contact Filter Search Test")
    public void callLogMultiselectFilterSearchTest(String fieldName, String filterType, String filterValue, String dbField, String expectedResult,String fieldType, String testCaseId) {
        FilterSearchReporter.logInfo("Test Case ID: ", testCaseId);
        FilterSearchReporter.logInfo("Account: ",email);
        JSONObject payload = createMultiselectAndDropdownFilterSearchPayload(fieldName, filterType, filterValue, dbField, fieldType);
        FilterSearchReporter.logPayload(payload);
    ...
    public JSONObject createMultiselectAndDropdownFilterSearchPayload(String fieldName, String filterType, String filterValue, String dbField, String fieldType) {
        JSONObject payload = new JSONObject();
        payload.put("advancedSearchContext", "CONTACT");
        ...
        JSONObject filterValueObj = null;
        if (dbField.equals("calltype")) {
    ...
    public void createTestData() {
        ...
        try {
            ...
            List<Integer> callLogIds = createCallLogForContact(contactSlugMap);
        ...
        try {
            ...
            for (String contactKey : contactSlugMap.keySet()) {
                ...
                futures.add(CompletableFuture.runAsync(() -> {
                    ...
                    JSONObject payload = processPayload(callLogsData, contactSlug);
    ...
    private JSONObject processPayload(JSONObject payload, String contactSlug) {
        if (payload.has("calls") && payload.get("calls") instanceof JSONArray) {
            ...
            for (int i = 0; i < callsArray.length(); i++) {
                ...
                // First ensure all fields exist (set empty if missing)
                for (String field : associatedFields) {
                    if (!call.has(field)) {
                        call.put(field, "");
                    }
                }
                // Then process non-empty fields that contain placeholders
                for (String field : associatedFields) {
                    if (call.has(field) && !call.getString(field).isEmpty()) {
    ...
    private void processAssociatedEntityField(JSONObject callLog, String fieldName) {
        if (callLog.has(fieldName)) {
            ...
            // Check if the value contains any associated entity placeholder (e.g., {associated_contacts_contact1})
            // Note: In contact_data.json, fields are swapped - associated_companies contains contact placeholders
            if (fieldValue != null && !fieldValue.isEmpty() && fieldValue.startsWith("{") && fieldValue.contains("associated_")) {
    ...
    public void updateCallLogWithTimestampScenarios(List<Integer> callLogIds) {
        ...
        int callLogIndex = 0;
        for (Map.Entry<String, Map<String, String>> scenario : callLogTimestampScenarios.entrySet()) {
            if (callLogIndex >= callLogIds.size()) {
                break;
            }

            Map<String, String> timestamps = scenario.getValue();
            Integer callLogId = callLogIds.get(callLogIndex);
