...
public class CrossAccountWorkHistorySecurityTest extends TestBase {
    ...
    @Owner("Raj Pandey")
    @Test(dataProvider = "crossAccountWorkHistoryTestData", groups = {"candidate_service", "nightly-build"})
    public void crossAccountWorkHistoryOperations_Test(String testScenario, String accountType, String tokenType, String operation, String expectedStatusCode, String expectedResponse, String description) {
        ...
        switch (operation.toUpperCase()) {
            case "GET_WORK_HISTORY_A":
                pathParams.put("candidateId", candidateIdA);
                response = RestClient.doGet("JSON", candidatesURL, "candidates/{candidateId}/work-history", 
                    token, null, pathParams, true);
                break;
                
            case "GET_WORK_HISTORY_B":
    ...
    @DataProvider(name = "crossAccountWorkHistoryTestData")
    public static Object[][] crossAccountWorkHistoryTestData() {
        return new Object[][] {
            {"ACCOUNT_A_OWN_DATA", "AccountA", "valid", "GET_WORK_HISTORY_A", "200", "success", "Account A should access its own candidate's work history"},
            {"ACCOUNT_B_ACCESS_A_DATA", "AccountB", "valid", "GET_WORK_HISTORY_A", "404", "access_denied", "Account B should not access Account A's candidate data"},
            {"ACCOUNT_A_INVALID_TOKEN", "AccountA", "invalid", "GET_WORK_HISTORY_A", "401", "invalid_token", "Account A should be denied with invalid token"}
        };
    }
