...
public class CrossAccountGetOffLimitStatusTest extends TestBase {

    private int entityTypeId = 5; // Candidates entity type
    ...
    private void createTestDataAccountA() {
        ...
        // Step 3: Mark candidate as off-limit in Account A
        markCandidateAsOffLimitAccountA(candidateIdAccountA, statusId);
    }
    ...
    private void markCandidateAsOffLimitAccountA(int candidateId, int statusId) {
        // Mark candidate as off-limit in Account A
        ...
        markOffLimit.setReason("Test reason for marking candidate as off-limit in Account A");
        ...
        assertThat("Failed to mark candidate as off-limit in Account A", response.getStatusCode(), equalTo(200));
    }

    @Owner("Raj Pandey")
    @Test(dataProvider = "crossAccountOffLimitStatusTestData", groups = {"candidate_service", "nightly-build"})
        ...
        try {
            switch (operation.toUpperCase()) {
                case "GET_OFF_LIMIT_STATUS_CROSS_ACCOUNT":
                    ...
                    response = getOffLimitStatus(entityTypeId, candidateIdAccountA, token);
                    break;
            }
    ...
    private Response getOffLimitStatus(int entityTypeId, int candidateId, String token) {
        Map<String, String> pathParams = new HashMap<>();
        pathParams.put("entityTypeId", String.valueOf(entityTypeId));
        pathParams.put("id", String.valueOf(candidateId));

        return RestClient.doGet("JSON", candidatesURL, "off-limit/entity/{entityTypeId}/status/{id}",
