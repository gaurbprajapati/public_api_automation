...
public class OwnerFilterCandidateTest extends FilterSearchBaseTest {
    ...
    @Owner("Yash Rampal")
    @Test(dataProvider = "ownerFilterTestData", description = "Filter Search Test for Owner Field")
    public void ownerFilterTest(String fieldName, String filterType, String filterValue, String dbField, String expectedResult) {
        ...
        Response response = executeFilterSearch(payload, ownerToken, "candidates");
    ...
    @DataProvider(name = "ownerFilterTestData")
    public Object[][] dataProvider() {
        JSONObject filterData = readJsonFileFromPath("src/test/resources/filtersDataProvider/candidateOwnerFilterDataProvider.json");
    ...
    @Owner("Raj Pandey")
    @Test(dataProvider = "isEmptyOwnerFilterData", description = "Test is_empty filter for candidates with no owner")
    public void testIsEmptyOwnerFilter(String fieldName, String filterType, String filterValue, String dbField, String expectedResult) {
        JSONObject payload = createFilterSearchPayload(fieldName, filterType, filterValue, dbField);
        ...
        validateExpectedUsers(data, expectedResult, filterType);
    }

    @DataProvider(name = "isEmptyOwnerFilterData")
    public Object[][] isEmptyOwnerFilterDataProvider() {
        createCandidateWithNoOwner();
        
        return new Object[][] {
            {"Owner", "is_empty", "", "ownerid", "0"}
        };
    }

    private void createCandidateWithNoOwner() {
        Response response = function.createCandidate(albatrossURL, ownerToken);
        Assert.assertEquals(response.getStatusCode(), 200);
        
        JsonPath jsonPath = response.jsonPath();
        Map<String, Object> candidateDataMap = jsonPath.get("data.candidate");
        JSONObject fullCandidateData = new JSONObject(candidateDataMap);
        
        transferOwnershipToNone(fullCandidateData);
    }
    
    private void transferOwnershipToNone(JSONObject fullCandidateData) {
        String basePath = "users/transfer-ownership/0";
        
        JSONObject payload = new JSONObject();
        payload.put("relatedtotypeid", 5);
        payload.put("selectedowner", 0);
        
        JSONArray selectedRows = new JSONArray();
        selectedRows.put(fullCandidateData);
        payload.put("selectedrows", selectedRows);

        Response response = RestClient.doPost1("JSON", albatrossURL, basePath, ownerToken, null, null, true, payload);
        Assert.assertEquals(response.getStatusCode(), 200);
    }
    ...
    private void createTestTeam() {
        ...
        Response createTeamResponse = function.createTeam(albatrossURL, ownerToken, "TestTeam_" + System.currentTimeMillis(), userIds);
    ...
    public void createTestData() {
        ...
        for (int i = 0; i < Math.min(candidateKeys.size(), testUserIds.size()) && i < 4; i++) {
            ...
            String userToken = userTokenMap.get(userId);
            ...
            payload.put("emailid", "owner_test_" + userId + "_" + System.currentTimeMillis() + "@yopmail.com");
            
            Response response = function.createCandidateWithJson(albatrossURL, userToken, payload);
            Assert.assertEquals(response.getStatusCode(), 200, "Failed to create candidate for user " + userId);
        }
    }
    ...
    private void validateExpectedUsers(JSONArray data, String expectedResult, String filterType) {
        ...
        for (int i = 0; i < data.length(); i++) {
            ...
            actualIdSet.add(String.valueOf(candidate.getInt("ownerid")));
        }
        
        Assert.assertEquals(actualIdSet, expectedIdSet, 
            "Expected owners " + expectedIdSet + " but found " + actualIdSet + " for filter: " + filterType);
    }
}